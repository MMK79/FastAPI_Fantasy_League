...
# run the file by pytest test_crud.py
import pytest
...
# Decorator + fixture -> prepare the testing setup during arrange phase -> uses function scope = run each time for each function
@pytest.fixture(scope="function")
# Create database session
def db_session():
    """This starts a database session and closes it when done"""
    session = SessionLocal()
    # through yeild -> pause when the test function uses the session
    yield session
    # close the session when the test is complete
    session.close()
...
def tests_get_all_performances(db_session):
