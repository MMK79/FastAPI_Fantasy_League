...
## Prediction
Get prediction of player acquisition cost.
"""

# The code is not in function definition so it will run once at startup of the API

# Load the ONNX models
# these session objects are a way to make inferences
sess_10 = rt.InferenceSession(
    "acquisition_model_10.onnx", providers=["CPUExecutionProvider"]
)
sess_50 = rt.InferenceSession(
    "acquisition_model_50.onnx", providers=["CPUExecutionProvider"]
)
sess_90 = rt.InferenceSession(
    "acquisition_model_90.onnx", providers=["CPUExecutionProvider"]
)

# Get the input and output names of the model from sessions
...
# defines a GET endpoint at the root address
@app.get(
    "/",
    summary="Check to see if the Fantasy acquisition API is running",
    description="""
    Use this endpoint to check if the API is running. You can also check it first before making other calls to be sure it's running.
    """,
...
@app.post(
    "/predict/",
    # define the return type of this endpoint
    ...
    A JSON record three predicted amounts.
    Together they give a possible range of acquisition costs for a player.
    """,
...
def predict(features: FantasyAcquisitionFeatures):
    ...
    # reformat the input variables into a NumPy array -> expected by ONNX runtime
    input_data = np.array(
        [
            [
    ...
    # Calls ONNX Runtime gets inference  using the input from API call
    ...
    return PredictionOutput(
        # create PredictionOutput object with inference values and return it in the API call
